...
const WIN_SEGMENT = 0;
const FULL_SPINS = 6;
const LABELS = ["6x", "0x", "0x", "0x", "0x", "0x", "0x", "0x"];

function buildWheelBackground() {
  const stops: string[] = [];
  for (let i = 0; i < SEGMENTS; i++) {
    const color =
      i === WIN_SEGMENT ? "hsl(45 90% 55%)" : i % 2 === 0 ? "hsl(240 6% 14%)" : "hsl(240 6% 22%)";
...
function targetRotation(outcome: "win" | "lose") {
  ...
  if (outcome === "lose") index = 1 + Math.floor(Math.random() * (SEGMENTS - 1));
        ...
        {Array.from({ length: SEGMENTS }).map((_, i) => {
          ...
          return (
            <div key={i}>
              ...
              <span
                className={cn(
                  "absolute left-1/2 top-1/2 origin-top text-[11px] font-black",
                  i === WIN_SEGMENT ? "text-background" : "text-white/85",
                )}
