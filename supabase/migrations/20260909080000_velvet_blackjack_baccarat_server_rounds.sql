-- Extends the authoritative-settlement system (see 20260908000000,
-- 20260909030000 and 20260909070000) to cover the two newly added Betrix
-- Originals table games - velvet-blackjack (Blackjack Royale) and
-- velvet-baccarat (Mega Baccarat) - linked to the real wallet the same way
-- the other eight Betrix Originals games already are. Additive only: every
-- previously-allowed game id stays allowed.
...
alter table public.velvet_sessions add constraint velvet_sessions_game_id_check
  check (game_id in ('velvet-vault', 'velvet-roulette', 'velvet-candy', 'velvet-thunder', 'velvet-sugar', 'velvet-paw', 'velvet-bass', 'velvet-grand', 'velvet-blackjack', 'velvet-baccarat'));
...
begin
 ...
 if p_game not in ('velvet-vault','velvet-roulette','velvet-candy','velvet-thunder','velvet-sugar','velvet-paw','velvet-bass','velvet-grand','velvet-blackjack','velvet-baccarat') then raise exception 'Unknown game';end if;
...
-- Old generic entrypoints must not settle a Betrix Originals game under
-- different rules. This guardrail (originally shipped for the first velvet
-- games) already exists on this project, but double-check it's still
-- correct rather than assume: if the check line is somehow missing from
-- these two legacy entrypoints, restore it, without touching any other
-- behavior in the function.
do $$
declare fn regprocedure; definition text;
begin
 for fn in select p.oid::regprocedure from pg_proc p join pg_namespace n on n.oid=p.pronamespace
 where n.nspname='public' and p.proname in ('play_casino_round','play_html5_casino_round')
 loop
  definition:=pg_get_functiondef(fn);
  if definition not ilike '%Use the Velvet game service%' then
    definition:=regexp_replace(definition,'begin',E'begin\n  if _game_id like ''velvet-%'' then raise exception ''Use the Velvet game service''; end if;','i');
    execute definition;
  end if;
 end loop;
end $$;
