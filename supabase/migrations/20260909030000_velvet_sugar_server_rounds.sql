-- Extends the existing authoritative-settlement system (see
-- 20260908000000_velvet_server_rounds.sql) to cover velvet-sugar (Sugar
-- Spark), which shipped as a client-only demo with its own fake balance and
-- is now being linked to the real wallet the same way the other four
-- Betrix Originals games already are: the browser only ever sends intent
-- (stake), the server resolves the spin, and settle_velvet_round is the
-- single place that ever touches a wallet balance for these games.
...
alter table public.velvet_sessions add constraint velvet_sessions_game_id_check
  check (game_id in ('velvet-vault', 'velvet-roulette', 'velvet-candy', 'velvet-thunder', 'velvet-sugar'));
...
begin
 ...
 if p_game not in ('velvet-vault','velvet-roulette','velvet-candy','velvet-thunder','velvet-sugar') then raise exception 'Unknown game';end if;
